...
public class InterfaceCreator {
    ...
    public Consumer<List<Integer>> addEvenValuesAtTheEnd() {
        return list -> list.addAll(list.stream().filter(n -> n % 2 == 0).collect(Collectors.toList()));
    }

    public Supplier<List<String>> filterCollection(List<String> values) {
        return () -> values.stream().filter(s -> s.matches("[A-Z]\\S*\\s+\\w+\\s+\\w+\\s+\\w+.*\\.")).collect(Collectors.toList());
    }
